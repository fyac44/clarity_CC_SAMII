...
% Set audiogram
ag_fs_ha = [125 250 500 1e3 2e3 4e3 8e3];
ag_dbloss_ha = [0 0 0 20 40 60 80]; % Example high-freq hearing loss
...
trefmin = 208.5e-6; % Minimun refractory period possible in BEZ2018.
                    % As min(tabs).
...
[ha_l_spikes,t_ha_spikes] = ...
    generate_neurogram_BEZ2018_CUDA(stim, Fs_stim, species,...
    ag_fs_ha, ag_dbloss_ha, CFs, ...
    numsponts_healthy);
disp('Computing HA R')
[ha_r_spikes,~] = ...
    generate_neurogram_BEZ2018_CUDA(stim, Fs_stim, species,...
...
[left_mi, right_mi, binaural_mi, left_pi, right_pi, binaural_pi, left_ti, right_ti, binaural_ti] = ...
    ...
    sum(numsponts_healthy), N, hop, i_frames, min_size, numcfs, total_frames);
...
% fout = [output_path filesep signal.signal '.json'];
% fid = fopen(fout, 'wt');
% fprintf(fid,"%s",txt);
